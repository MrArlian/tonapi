# This file is auto-generated. Do not edit manually.

from pytonapi.rest.models._enums import (
    AccountStatus,
    AccStatusChange,
    BouncePhaseType,
    ComputeSkipReason,
    CurrencyType,
    ExecGetMethodArgType,
    JettonVerificationType,
    PoolImplementationType,
    TransactionType,
    TrustType,
)
from pytonapi.rest.models.accounts import (
    Account,
    AccountAddress,
    AccountEvent,
    AccountEvents,
    Accounts,
    Action,
    ActionSimplePreview,
    AddExtensionAction,
    AuctionBidAction,
    ContractDeployAction,
    DepositStakeAction,
    DepositTokenStakeAction,
    DnsExpiring,
    DomainNames,
    DomainRenewAction,
    ElectionsDepositStakeAction,
    ElectionsRecoverStakeAction,
    EncryptedComment,
    ExecGetMethodArg,
    ExtraCurrencies,
    ExtraCurrencyTransferAction,
    FlawedJettonTransferAction,
    FoundAccounts,
    GasRelayAction,
    JettonBalance,
    JettonBurnAction,
    JettonMintAction,
    JettonOperation,
    JettonOperations,
    JettonPreview,
    JettonsBalances,
    JettonSwapAction,
    JettonTransferAction,
    LiquidityDepositAction,
    Multisigs,
    NftItemTransferAction,
    NftPurchaseAction,
    Protocol,
    PurchaseAction,
    Refund,
    RemoveExtensionAction,
    SetSignatureAllowedAction,
    SmartContractAction,
    Subscription,
    SubscriptionAction,
    Subscriptions,
    TonTransferAction,
    TraceID,
    TraceIDs,
    UnSubscriptionAction,
    VaultDepositInfo,
    WithdrawStakeAction,
    WithdrawStakeRequestAction,
    WithdrawTokenStakeRequestAction,
)
from pytonapi.rest.models.blockchain import (
    AccountStorageInfo,
    ActionPhase,
    BlockchainAccountInspect,
    BlockchainBlock,
    BlockchainBlocks,
    BlockchainBlockShards,
    BlockchainConfig,
    BlockchainLibrary,
    BlockchainRawAccount,
    BlockCurrencyCollection,
    BlockLimits,
    BlockParamLimits,
    BlockValueFlow,
    ComputePhase,
    ConfigProposalSetup,
    CreditPhase,
    Error,
    ExtraCurrency,
    GasLimitPrices,
    JettonBridgeParams,
    JettonBridgePrices,
    Message,
    Method,
    MethodExecutionResult,
    MisbehaviourPunishmentConfig,
    MsgForwardPrices,
    Oracle,
    OracleBridgeParams,
    RawBlockchainConfig,
    ReducedBlock,
    ReducedBlocks,
    SizeLimitsConfig,
    Source,
    SourceFile,
    StateInit,
    StoragePhase,
    Transaction,
    Transactions,
    TvmStackRecord,
    Validator,
    Validators,
    ValidatorsSet,
    WorkchainDescr,
)
from pytonapi.rest.models.connect import AccountInfoByStateInit
from pytonapi.rest.models.dns import (
    Auction,
    Auctions,
    DnsRecord,
    DomainBid,
    DomainBids,
    DomainInfo,
    PictureDNS,
    WalletDNS,
)
from pytonapi.rest.models.emulation import DecodedMessage, DecodedRawMessage
from pytonapi.rest.models.events import Event, ValueFlow
from pytonapi.rest.models.extra_currency import EcPreview
from pytonapi.rest.models.gasless import (
    GaslessConfig,
    GaslessTx,
    SignRawMessage,
    SignRawParams,
)
from pytonapi.rest.models.jettons import (
    JettonHolders,
    JettonInfo,
    JettonMetadata,
    Jettons,
    JettonTransferPayload,
    ScaledUI,
)
from pytonapi.rest.models.lite_server import BlockRaw, InitStateRaw
from pytonapi.rest.models.multisig import JettonQuantity, Multisig, MultisigOrder, Risk
from pytonapi.rest.models.nft import (
    ImagePreview,
    NftApprovedBy,
    NftCollection,
    NftCollections,
    NftItem,
    NftItems,
    NftOperation,
    NftOperations,
    Sale,
)
from pytonapi.rest.models.purchases import AccountPurchases, Metadata, Price, Purchase
from pytonapi.rest.models.rates import ChartPoints, MarketTonRates, TokenRates
from pytonapi.rest.models.staking import (
    AccountStaking,
    AccountStakingInfo,
    ApyHistory,
    PoolImplementation,
    PoolInfo,
)
from pytonapi.rest.models.storage import StorageProvider
from pytonapi.rest.models.traces import Trace
from pytonapi.rest.models.utilities import ServiceStatus
from pytonapi.rest.models.wallet import (
    MessageConsequences,
    Seqno,
    Wallet,
    WalletPlugin,
    Wallets,
    WalletsByPublicKey,
    WalletsByPublicKeys,
    WalletStats,
)

# Rebuild models that use forward references from TYPE_CHECKING imports
_models_to_rebuild = [
    Account,
    AccountAddress,
    AccountEvent,
    AccountEvents,
    Accounts,
    Action,
    ActionSimplePreview,
    AddExtensionAction,
    AuctionBidAction,
    ContractDeployAction,
    DepositStakeAction,
    DepositTokenStakeAction,
    DnsExpiring,
    DomainNames,
    DomainRenewAction,
    ElectionsDepositStakeAction,
    ElectionsRecoverStakeAction,
    EncryptedComment,
    ExecGetMethodArg,
    ExtraCurrencies,
    ExtraCurrencyTransferAction,
    FlawedJettonTransferAction,
    FoundAccounts,
    GasRelayAction,
    JettonBalance,
    JettonBurnAction,
    JettonMintAction,
    JettonOperation,
    JettonOperations,
    JettonPreview,
    JettonSwapAction,
    JettonTransferAction,
    JettonsBalances,
    LiquidityDepositAction,
    Multisigs,
    NftItemTransferAction,
    NftPurchaseAction,
    Protocol,
    PurchaseAction,
    Refund,
    RemoveExtensionAction,
    SetSignatureAllowedAction,
    SmartContractAction,
    Subscription,
    SubscriptionAction,
    Subscriptions,
    TonTransferAction,
    TraceID,
    TraceIDs,
    UnSubscriptionAction,
    VaultDepositInfo,
    WithdrawStakeAction,
    WithdrawStakeRequestAction,
    WithdrawTokenStakeRequestAction,
    AccountStorageInfo,
    ActionPhase,
    BlockCurrencyCollection,
    BlockLimits,
    BlockParamLimits,
    BlockValueFlow,
    BlockchainAccountInspect,
    BlockchainBlock,
    BlockchainBlockShards,
    BlockchainBlocks,
    BlockchainConfig,
    BlockchainLibrary,
    BlockchainRawAccount,
    ComputePhase,
    ConfigProposalSetup,
    CreditPhase,
    Error,
    ExtraCurrency,
    GasLimitPrices,
    JettonBridgeParams,
    JettonBridgePrices,
    Message,
    Method,
    MethodExecutionResult,
    MisbehaviourPunishmentConfig,
    MsgForwardPrices,
    Oracle,
    OracleBridgeParams,
    RawBlockchainConfig,
    ReducedBlock,
    ReducedBlocks,
    SizeLimitsConfig,
    Source,
    SourceFile,
    StateInit,
    StoragePhase,
    Transaction,
    Transactions,
    TvmStackRecord,
    Validator,
    Validators,
    ValidatorsSet,
    WorkchainDescr,
    AccountInfoByStateInit,
    Auction,
    Auctions,
    DnsRecord,
    DomainBid,
    DomainBids,
    DomainInfo,
    PictureDNS,
    WalletDNS,
    DecodedMessage,
    DecodedRawMessage,
    Event,
    ValueFlow,
    EcPreview,
    GaslessConfig,
    GaslessTx,
    SignRawMessage,
    SignRawParams,
    JettonHolders,
    JettonInfo,
    JettonMetadata,
    JettonTransferPayload,
    Jettons,
    ScaledUI,
    BlockRaw,
    InitStateRaw,
    JettonQuantity,
    Multisig,
    MultisigOrder,
    Risk,
    ImagePreview,
    NftCollection,
    NftCollections,
    NftItem,
    NftItems,
    NftOperation,
    NftOperations,
    Sale,
    AccountPurchases,
    Metadata,
    Price,
    Purchase,
    MarketTonRates,
    TokenRates,
    AccountStaking,
    AccountStakingInfo,
    ApyHistory,
    PoolImplementation,
    PoolInfo,
    StorageProvider,
    Trace,
    ServiceStatus,
    MessageConsequences,
    Seqno,
    Wallet,
    WalletPlugin,
    WalletStats,
    Wallets,
    WalletsByPublicKey,
    WalletsByPublicKeys,
]
for _model in _models_to_rebuild:
    _model.model_rebuild()  # type: ignore[attr-defined]

__all__ = [
    "AccStatusChange",
    "Account",
    "AccountAddress",
    "AccountEvent",
    "AccountEvents",
    "AccountInfoByStateInit",
    "AccountPurchases",
    "AccountStaking",
    "AccountStakingInfo",
    "AccountStatus",
    "AccountStorageInfo",
    "Accounts",
    "Action",
    "ActionPhase",
    "ActionSimplePreview",
    "AddExtensionAction",
    "ApyHistory",
    "Auction",
    "AuctionBidAction",
    "Auctions",
    "BlockCurrencyCollection",
    "BlockLimits",
    "BlockParamLimits",
    "BlockRaw",
    "BlockValueFlow",
    "BlockchainAccountInspect",
    "BlockchainBlock",
    "BlockchainBlockShards",
    "BlockchainBlocks",
    "BlockchainConfig",
    "BlockchainLibrary",
    "BlockchainRawAccount",
    "BouncePhaseType",
    "ChartPoints",
    "ComputePhase",
    "ComputeSkipReason",
    "ConfigProposalSetup",
    "ContractDeployAction",
    "CreditPhase",
    "CurrencyType",
    "DecodedMessage",
    "DecodedRawMessage",
    "DepositStakeAction",
    "DepositTokenStakeAction",
    "DnsExpiring",
    "DnsRecord",
    "DomainBid",
    "DomainBids",
    "DomainInfo",
    "DomainNames",
    "DomainRenewAction",
    "EcPreview",
    "ElectionsDepositStakeAction",
    "ElectionsRecoverStakeAction",
    "EncryptedComment",
    "Error",
    "Event",
    "ExecGetMethodArg",
    "ExecGetMethodArgType",
    "ExtraCurrencies",
    "ExtraCurrency",
    "ExtraCurrencyTransferAction",
    "FlawedJettonTransferAction",
    "FoundAccounts",
    "GasLimitPrices",
    "GasRelayAction",
    "GaslessConfig",
    "GaslessTx",
    "ImagePreview",
    "InitStateRaw",
    "JettonBalance",
    "JettonBridgeParams",
    "JettonBridgePrices",
    "JettonBurnAction",
    "JettonHolders",
    "JettonInfo",
    "JettonMetadata",
    "JettonMintAction",
    "JettonOperation",
    "JettonOperations",
    "JettonPreview",
    "JettonQuantity",
    "JettonSwapAction",
    "JettonTransferAction",
    "JettonTransferPayload",
    "JettonVerificationType",
    "Jettons",
    "JettonsBalances",
    "LiquidityDepositAction",
    "MarketTonRates",
    "Message",
    "MessageConsequences",
    "Metadata",
    "Method",
    "MethodExecutionResult",
    "MisbehaviourPunishmentConfig",
    "MsgForwardPrices",
    "Multisig",
    "MultisigOrder",
    "Multisigs",
    "NftApprovedBy",
    "NftCollection",
    "NftCollections",
    "NftItem",
    "NftItemTransferAction",
    "NftItems",
    "NftOperation",
    "NftOperations",
    "NftPurchaseAction",
    "Oracle",
    "OracleBridgeParams",
    "PictureDNS",
    "PoolImplementation",
    "PoolImplementationType",
    "PoolInfo",
    "Price",
    "Protocol",
    "Purchase",
    "PurchaseAction",
    "RawBlockchainConfig",
    "ReducedBlock",
    "ReducedBlocks",
    "Refund",
    "RemoveExtensionAction",
    "Risk",
    "Sale",
    "ScaledUI",
    "Seqno",
    "ServiceStatus",
    "SetSignatureAllowedAction",
    "SignRawMessage",
    "SignRawParams",
    "SizeLimitsConfig",
    "SmartContractAction",
    "Source",
    "SourceFile",
    "StateInit",
    "StoragePhase",
    "StorageProvider",
    "Subscription",
    "SubscriptionAction",
    "Subscriptions",
    "TokenRates",
    "TonTransferAction",
    "Trace",
    "TraceID",
    "TraceIDs",
    "Transaction",
    "TransactionType",
    "Transactions",
    "TrustType",
    "TvmStackRecord",
    "UnSubscriptionAction",
    "Validator",
    "Validators",
    "ValidatorsSet",
    "ValueFlow",
    "VaultDepositInfo",
    "Wallet",
    "WalletDNS",
    "WalletPlugin",
    "WalletStats",
    "Wallets",
    "WalletsByPublicKey",
    "WalletsByPublicKeys",
    "WithdrawStakeAction",
    "WithdrawStakeRequestAction",
    "WithdrawTokenStakeRequestAction",
    "WorkchainDescr",
]
