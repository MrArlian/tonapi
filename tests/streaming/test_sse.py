...
class TestSSE(TestTonapiStreaming):
    async def test_subscribe_transactions(self) -> None:
        count = 0
        async for event in self.streaming.sse.subscribe_transactions(
            accounts=[ACCOUNT_ID],
            ...
            count += 1
        print(f"SSE transactions: {count} events")

    async def test_subscribe_blocks(self) -> None:
        count = 0
        async for block in self.streaming.sse.subscribe_blocks(
            ...
            count += 1
        print(f"SSE blocks: {count} events")

    async def test_subscribe_traces(self) -> None:
        count = 0
        async for trace in self.streaming.sse.subscribe_traces(
            accounts=[ACCOUNT_ID],
            stop=self.make_stop(DURATION),
        ):
            ...
            count += 1
        print(f"SSE traces: {count} events")

    async def test_subscribe_mempool(self) -> None:
        count = 0
        async for msg in self.streaming.sse.subscribe_mempool(
            stop=self.make_stop(DURATION),
        ):
            self.assertIsInstance(msg, MempoolEvent)
            self.assertTrue(msg.boc)
            count += 1
        print(f"SSE mempool: {count} events")
